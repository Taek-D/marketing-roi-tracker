...
[Google Ads API] ────┐
                     │
[Facebook Ads API] ──┼──→ [automation/Code.gs: main()] ──→ [Raw Data Sheet] ──→ [automation/Attribution.gs] ──→ [Dashboard Sheet]
                     │          │
...
| 파일 | 역할 | 의존성 |
|------|------|--------|
| automation/Code.gs | 엔트리포인트, API 호출, Slack 알림 | Config.gs |
| automation/Config.gs | 전역 설정(CONFIG), 유틸리티(getProperty, log) | 없음 |
| automation/Attribution.gs | ROAS 계산, 대시보드 갱신 | Config.gs |
| automation/Setup.gs | 초기 설정 (1회 실행용) | Config.gs, Attribution.gs |
